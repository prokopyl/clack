...
impl Entry for GainPluginEntry {
    fn new(_bundle_path: &CStr) -> Result<Self, EntryLoadError> {
        ...
        Ok(Self {
            ...
            preset_discovery_factory: PresetDiscoveryFactoryWrapper::new(
                GainPresetDiscoveryFactory::new(),
            ),
        })
    }
    ...
    fn declare_factories<'a>(&'a self, builder: &mut EntryFactories<'a>) {
        builder
            ...
            .register_factory(&self.preset_discovery_factory);
    }
}
...
impl PluginFactoryImpl for GainPluginFactory {
    ...
    fn create_plugin<'a>(
        ...
        if plugin_id != self.descriptor.id().unwrap() {
            return None;
        }
        ...
        Some(PluginInstance::new::<GainPlugin>(
            ...
            |_, shared| Ok(GainPluginMainThread { shared }),
        ))
    }
}

clack_export_entry!(GainPluginEntry);
