...
const _: () = {
    ...
    impl<H: PluginHoster + HostLatencyImpl> ExtensionImplementation<H> for HostLatency {
        const IMPLEMENTATION: &'static Self = &HostLatency(clap_host_latency {
            changed: Some(changed::<H>),
        });
    }

    unsafe extern "C" fn changed<H: PluginHoster + HostLatencyImpl>(host: *const clap_host) {
...
const _: () = {
    ...
    {
        const IMPLEMENTATION: &'static Self = &PluginLatency(clap_plugin_latency {
            get: Some(get::<P>),
        });
    }
