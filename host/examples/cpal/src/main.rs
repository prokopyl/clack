...
/// Procedures for discovering and loading CLAP plugin file.
mod discovery;
...
///
/// At least one of the `--plugin-id` (`-p`) or the `--file-path` (`-f`) parameters *must* be used
...
struct Cli {
    ...
    /// in conjunction with the `--file-path` (`-p`) parameter to specify which file to load the
    /// plugin from.
...
fn main() {
    ...
    let result = match (&args.file_path, &args.plugin_id) {
...
/// Loads the plugin contained in a file, given through its path.
///
/// Returns an error if there is more than one plugin in the file.
fn run_from_path(path: &Path) -> Result<(), Box<dyn Error>> {
    let entries = discovery::list_plugins_in_file(path)?;
    ...
    if entries.len() == 1 {
        let plugin = entries.into_iter().next().unwrap();
        host::run(plugin)
    } else {
...
fn run_from_id(id: &str) -> Result<(), Box<dyn Error>> {
    let entries = discovery::scan_for_plugin_id(id);

    if entries.is_empty() {
        return Err(MainError::NoPluginWithId(id.to_string()).into());
    }

    println!("Found {} CLAP plugins with id {}:", entries.len(), id);
...
/// Loads a specific plugin matching the given ID, from a specific file's path.
///
/// Returns an error if that specific plugin isn't present in the CLAP file.
fn run_specific(path: &Path, id: &str) -> Result<(), Box<dyn Error>> {
    let entry = discovery::load_plugin_id_from_path(path, id)?;
...
enum MainError {
    ...
    UnspecifiedOptions,
    /// There is no CLAP plugin in the file at this path.
    NoPluginInPath(PathBuf),
    /// No CLAP plugin with the given ID was found in the filesystem.
    NoPluginWithId(String),
    /// No CLAP plugin with the given ID was found in the file at the given path.
    NoPluginInPathWithId(PathBuf, String),
    /// There are multiple plugins in the given file, user needs to decide which one to load.
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self { MainError::UnspecifiedOptions => f.write_str("Please specify a plugin to load using the -p option or the -f option. Use --help for documentation."),
            MainError::NoPluginInPath(path) => write!(f,
                                                      "No plugins found in CLAP file at {}. Stopping.",
                                                      path.display()
            ),
            ...
            MainError::NoPluginInPathWithId(path, id) => write!(f,
                "Couldn't find a plugin matching id {id} in CLAP file at {}. Stopping.",
                path.display()),
            MainError::MultiplePluginsInPath(path) =>
                write!(f, "Found multiple plugins in CLAP file at {}. Specify a specific plugin ID using the -p option.", path.display()),
            MainError::MultiplePluginsWithId(id) => write!(f, "Found multiple plugins matching id {id}. Specify a specific file path using the -f option.")
        }
    }
}
